...
Private Sub UserForm_Initialize()
    ...
    ' 2. Rebuild cache if empty or if store count changed / Cache neu aufbauen falls veraltet
...
Public Sub PopulateTreeViewFromCache(ByVal filterText As String)
    ...
    For Each fData In gAllFolders
        ...
        ' Check if folder and store reference are still valid (PST might be removed)
        ' Prüfen, ob Ordner und Store-Referenz noch gültig sind (PST könnte entfernt worden sein)
        storeName = ""
        displayName = ""
        
        On Error Resume Next
        displayName = folder.Name
        If Not folder.store Is Nothing Then
            storeName = folder.store.displayName
        End If
        On Error GoTo EH
        
        ' Skip invalid/orphaned folders / Ungültige/entfernte Ordner überspringen
        If storeName <> "" And displayName <> "" Then
...
EH:
    ' Auto-fallback: If cache is corrupt/stale, clear cache and rebuild
    ' Automatischer Fallback: Falls Cache veraltet ist, neu aufbauen
    Err.Clear
    On Error Resume Next
    RefreshFolderCache
    On Error GoTo 0
